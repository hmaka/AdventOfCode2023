...
for b in broadcast:
    module = adaMap[b]
    if isinstance(module, Conjunction_module): module.parents["broadcast"] = False

key_modules = {"dt":None, "hh":None, "tn":None, "st":None,}
low_pulses = 0
high_pulses = 0
# begin simulation
pulse_queue = deque() 
...
while True:
    ...
    while pulse_queue:
        ...
        if prev in key_modules.keys() and pulse == True:
            key_modules[prev] = button_presses
            if all(key_modules.values()):
                print(key_modules.values())
                print(lcm(*key_modules.values()))
                exit(0)
